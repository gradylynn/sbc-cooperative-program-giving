...
The [Cooperative Program](https://www.sbc.net/missions/the-cooperative-program/) is the SBC's 
...
forwarded to the national Cooperative Program. The national Cooperative Program then budgets these resourses towards
national programs.
...
from each state convention and the budget items and amounts for each month. This data has been scraped and consolidated
[here](https://github.com/gradylynn/sbc-cooperative-program-giving/tree/main/data)
using original code from [here](https://github.com/gradylynn/sbc-cooperative-program-giving).

This project is intended as a general overview of the SBC's monthly Cooperative Program receipts and budget
(since October 2012).
...
There are **two parts of the data** which will be examined in turn:

1. **Budget** (amounts budgeted towards different missions and ministries)
...
We will first look at the national CP budget.
...
In this budget overview, the CP budget is divided into 4 broad categories:
...
##### Overall (Allocations + Designations) {.tabset .tabset-fade}
...
Note (from the previous charts) that the vast majority of these seminaries' budgets are from allocated, not designated.

##### Monthly Seminary Spending
```{r}
ggplot(data=temp_budget_df) +
